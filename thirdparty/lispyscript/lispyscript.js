...
(function(e, t, n) {
    ...
    return i
            ...
            tinix.cookies = {getItem: function(sKey) {
            ...
            var $ = require("tinix"), ls = require("lispyscript"), fs = require("fs"), cache = {}
            ...
            function createHandler(script) {
                return function(done) {
                    if (!script.src) {
                        ...
                        $.get(script.src, function(err, ls) {
                            if (err) {
            ...
            function runModule(js) {
                var module = {exports: {}}, func = new Function("module, exports, require", js), require = function(name) {
                    if (cache[name]) {
                        return cache[name]
                    } else {
                        if (window.require) {
            ...
            (function(__dirname) {
                var version = "0.3.6", banner = "// Generated by LispyScript v" + version + "\n", isWhitespace = /\s/, isFunction = /^function\b/, validName = /^[a-zA-Z_$][0-9a-zA-Z_$]*$/, noReturn = /^var\b|^set\b|^throw\b/, isHomoiconicExpr = /^#args-if\b|^#args-shift\b|^#args-second\b/, noSemiColon = false, indentSize = 4, indent = -indentSize, keywords = {}, macros = {}, errors = [], fs
                ...
                var parse = function(code, filename) {
                    code = "(" + code + ")"
                    var length = code.length, pos = 1, lineno = 1
                    var parser = function() {
                        var tree = [], token = "", isString = false, isSingleString = false, isJSArray = 0, isJSObject = 0, isListComplete = false, isComment = false, isRegex = false, isEscape = false, handleToken = function() {
                            if (token) {
                                tree.push(token)
                                token = ""
                            }
                        ...
                        if (isString)
                            handleError(3, tree._line, tree._filename)
                        if (isRegex)
                            handleError(14, tree._line, tree._filename)
                        if (isSingleString)
                            handleError(3, tree._line, tree._filename)
                        if (isJSArray > 0)
                            handleError(5, tree._line, tree._filename)
                        if (isJSObject > 0)
                            handleError(7, tree._line, tree._filename)
                        if (!isListComplete)
                            handleError(8, tree._line, tree._filename)
                        return tree
                    }
                    var ret = parser()
                    if (pos < length) {
                ...
                var macroExpand = function(tree) {
                    var command = tree[0], template = macros[command]["template"], code = macros[command]["code"], replacements = {}
                    ...
                    var replaceCode = function(source) {
                        ...
                        for (var i = 0; i < source.length; i++) {
                            if (typeof source[i] == "object") {
                                ...
                                var token = source[i], tokenbak = token, isATSign = false
                                if (token.indexOf("@") >= 0) {
                                    isATSign = true
                ...
                keywords["var"] = function(arr) {
                    if (arr.length < 3)
                        handleError(0, arr._line, arr._filename)
                    if (arr.length > 3) {
                        indent += indentSize
                    }
                    handleSubExpressions(arr)
                    ...
                    for (var i = 1; i < arr.length; i = i + 2) {
                        ...
                        if (!validName.test(arr[i]))
                            handleError(9, arr._line, arr._filename)
                        ret += arr[i] + ' = ' + arr[i + 1]
                    }
                    if (arr.length > 3) {
                        indent -= indentSize
                ...
                keywords["try"] = function(arr) {
                    if (arr.length < 3)
                        handleError(0, arr._line, arr._filename)
                    var c = arr.pop(), ind = " ".repeat(indent)
                ...
                var handleError = function(no, line, filename, extra) {
                    throw new Error(errors[no] + ((extra) ? " - " + extra : "") + ((line) ? "\nLine no " + line : "") + ((filename) ? "\nFile " + filename : ""))
                }
